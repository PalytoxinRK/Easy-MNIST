...
class CNN:
	...
	def useCNN(self,image,y):
		...
		y_index = y_.index(max(y_))
		labell_index = labell.index(max(labell))
...
def test():
	...
	for i in range(1000):
		image = matrix[i]
		labell = y[i]
		sucess = sucess + cnn.useCNN(image, labell)
		
		
	print('Sucess', sucess/1000*1.0)
